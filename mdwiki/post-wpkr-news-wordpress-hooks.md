# 워드프레스 오토 시스템에 자주 사용되는 워드프레스 액션/필터 훅(Hooks) 모음 - 워드프레스웹코리아
...
- [워드프레스 액션/필터 훅(Hooks) 활용 방법](#index-00)
- [1. 워드프레스 코어 (WordPress Core)](#index-01)
- [2. 우커머스 (WooCommerce)](#index-02)
- [3. 컨택트 폼 7 (Contact Form 7)](#index-03)
- [맺음말](#index-epilogue)
...
**워드프레스 오토 시스템(WordPress Auto System)**에서 생성한 실행 링크를 각 함수에 입력하면, 이벤트(예: 새로운 글 발행 후 실행)가 발생할 때, 해당 기능이 자동으로 실행됩니다.

사용중인 워드프레스 테마의 'functions.php'의 가장 마지막 부분에 붙여 넣습니다.
...
### (1) 새로운 글 발행 후 실행
```
function wptalk_wp_insert_post($postId, $post) {
  include( home_url(). '/auto-posting/' );
  file_get_contents( home_url().'/auto-posting/run/' );
}
add_action('wp_insert_post', 'wptalk_wp_insert_post', 10, 2);
```

### (2) 새로운 회원 가입 후 실행

```
function wptalk_user_register( $user_id ) {
  file_get_contents();
}
add_action( 'user_register', 'wptalk_user_register', 10, 1 );
```
...
```
function wptalk_woocommerce_payment_complete($order_id){
  file_get_contents();
}
add_action('woocommerce_payment_complete','wptalk_woocommerce_payment_complete');
```
...
```
function wptalk_after_sent_mail($wpcf7) {
  file_get_contents();
}
add_action('wpcf7_mail_sent', 'wptalk_after_sent_mail');
```

### (2) 컨택트 폼 7 플라밍고 데이터 저장 완료 후 실행

```
function wptalk_wpcf7_after_flamingo($data) {
  file_get_contents();
}
add_action('wpcf7_after_flamingo', 'wptalk_wpcf7_after_flamingo');
```
